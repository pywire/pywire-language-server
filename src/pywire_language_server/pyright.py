...
class PyrightClient:
    ...
    def __init__(self):
        ...
        self._response_callbacks: Dict[int, Callable[[Any], None]] = {}
        self._request_id = 0
    ...
    def _find_pyright_executable(self) -> Optional[str]:
        ...
        # Windows
        same_dir_exe_win = os.path.join(py_dir, "pyright-langserver.exe") # unlikely but possible
        ...
        src_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) # .../src
        project_root = os.path.dirname(src_root) # .../pywire-language-server
        
        # Unix
        sibling_venv_exe = os.path.join(project_root, ".venv", "bin", "pyright-langserver")
        if os.path.exists(sibling_venv_exe):
             return sibling_venv_exe

        # Windows
        sibling_venv_exe_win = os.path.join(project_root, ".venv", "Scripts", "pyright-langserver.exe")
    ...
    def _send(self, msg: Dict[str, Any]):
        ...
        body_bytes = f"Content-Length: {len(content_bytes)}\r\n\r\n".encode('utf-8') + content_bytes
